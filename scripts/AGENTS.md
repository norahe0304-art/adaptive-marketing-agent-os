...
validate_mounted_agents.py: Mounted agent 装配校验器；校验 agents/mounted/*.agent.md（或 --root/--glob 指向消费方 repo）是否接上 role、tenant attachment、work_substrate、entrypoints，且其 playbook 引用全在 base role 声明的 surface 内（声明与引用一致，无 tenant 特例）；空 mounted 优雅返回 0（spec repo）。
