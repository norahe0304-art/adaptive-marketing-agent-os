...
边界
mounted/ 是 assembly 层的占位与文档锚点。装配规则由 protocol-consumption.contract.md 定义；mounted agent = 一个 base role + 一个 tenant attachment + 命名 playbooks，adaptive/installable/detachable，runtime 由用户选择。schema 证明见 examples/，活体实例见消费方 repo。
